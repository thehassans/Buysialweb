...
export default function DriverLayout(){
  ...
  useEffect(()=>{ try{ localStorage.setItem('theme', theme) }catch{}; const root=document.documentElement; if(theme==='light') root.setAttribute('data-theme','light'); else root.removeAttribute('data-theme') },[theme])
  ...
  const mobileTabs = [
    { to: '/driver', label: 'Dashboard', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> },
  ]
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--fg)]">
      <div className={`main ${hideSidebar ? 'full-mobile' : (closed ? 'full' : '')} ${tabsVisible ? 'with-mobile-tabs' : ''}`}>
        ...
        <div className="topbar shadow-sm backdrop-blur supports-[backdrop-filter]:bg-opacity-90" style={{background:'var(--sidebar-bg)', borderBottom:'1px solid var(--sidebar-border)'}}>
          <div style={{display:'flex', alignItems:'center', gap:12, minHeight:48}}>
            ...
            <div style={{display:'inline-flex', alignItems:'center', gap:8, padding:'8px 12px', borderRadius:999, background:'var(--panel)', border:'1px solid var(--border)'}}>
              <span aria-hidden>🚚</span>
              <span style={{fontWeight:800, letterSpacing:0.3}}>Driver {me.firstName||''} {me.lastName||''}</span>
            </div>
          </div>
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            <button className="btn secondary" onClick={()=> setTheme(t=> t==='light' ? 'dark' : 'light')} title="Toggle theme">{theme==='light' ? '🌙 Dark' : '🌞 Light'}</button>
      ...
      {tabsVisible && (
        <nav className="mobile-tabs" role="navigation" aria-label="Primary">
          {mobileTabs.map(tab => (
            <NavLink key={tab.to} to={tab.to} end={tab.to === '/driver'} className={({isActive})=>`tab ${isActive?'active':''}`}>
